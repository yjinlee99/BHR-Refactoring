...
- 하나의 `Dept`에는 여러 `Employees`가 소속될 수 있다.
- 하나의 `Dept`는 한 명의 `Employees`를 부서장으로 참조한다.
- 한 명의 `Employees`는 여러 근태 기록과 연차 기록을 가질 수 있다.
- 한 명의 `Employees`는 여러 배지를 받을 수 있다.
- 하나의 `BadgeMaster`도 여러 직원에게 부여될 수 있다.
...
부서장 정보를 단순 문자열이나 사번으로 저장하지 않고 실제 `Employees` 엔티티와 연결하는 구조이다.

다만 현재 관계만으로는 **부서장으로 지정된 직원이 반드시 해당 부서 소속인지까지는 보장하지 않는다.**

필요하다면 이후 Service 계층에서 해당 조건을 검증할 수 있다.
...
이를 직접 `N : M`으로 연결하지 않고 `EmpBadge`라는 중간 엔티티를 사용한다.

```text
Employees 1 ── N EmpBadge N ── 1 BadgeMaster
```

`EmpBadge`는 직원과 배지를 연결하는 것뿐만 아니라 배지의 **부여일과 종료일**도 저장한다.
...
이처럼 두 엔티티의 관계 자체에 별도로 저장해야 할 정보가 있으면 연결 엔티티로 표현할 수 있다.

---

#### `Annual`과 `AnnualList`

두 엔티티는 모두 연차와 관련되어 있지만 역할이 다르다.
...
예를 들어 `Annual`에는 다음과 같은 정보가 저장된다.
...
반면 `AnnualList`에는 실제 사용 기록이 여러 건 저장될 수 있다.

```text
직원 A / 2026년 / 1월 10일 / 1일 사용
직원 A / 2026년 / 3월 20일 / 0.5일 사용
```

따라서 한 직원에게 같은 연도의 `Annual`은 한 건만 필요하지만, `AnnualList`는 여러 건 존재할 수 있다.

---

#### `Annual` 복합키

현재 `Annual`은 다음 두 값을 묶어 한 행을 식별한다.

```text
직원 + 연도
```

예를 들어 다음 데이터는 서로 다른 데이터이다.
...
하지만 같은 직원과 같은 연도의 데이터는 두 개 존재하면 안 된다.
...
따라서 `Annual`의 복합키가 표현하는 업무 규칙은 다음과 같다.
...
현재 `Annual`은 `@IdClass(AnnualPK.class)`를 사용하고 있다.

코드를 확인한 결과 기존 `AnnualPK`는 다음과 같이 작성되어 있었다.
...
복합키 클래스를 확인하면서 다음 두 가지 개선점을 발견했다.

1. `@IdClass`로 사용하는 식별자 클래스이지만 `Serializable`을 구현하지 않고 있다.
2. `employees` 필드가 `Employees` 타입으로 선언되어 있다.

`Employees`의 PK 타입은 `String`이므로 다음과 같이 수정할 수 있다.
...
이를 통해 `Annual`의 복합키는 실제로 다음 값을 기준으로 구성된다.

```text
annualYear + Employees의 PK(emp_id)
```

기존 기능에서는 해당 구조가 문제없이 동작했지만, 복합키 매핑을 다시 확인하면서 JPA 식별자 클래스 규칙에 맞지 않는 부분을 발견했고 리팩토링 대상으로 정리했다.

---

#### 다른 설계 방법

현재처럼 복합키를 사용하는 대신 별도의 단일 PK를 추가할 수도 있다.
...
그리고 다음 두 컬럼의 조합에 Unique 제약을 설정하는 방식이다.

```text
UNIQUE(employee_id, annual_year)
```

또는 `@EmbeddedId`, `@MapsId`를 이용해 복합키를 별도의 객체로 표현할 수도 있다.

어떤 방식을 사용하는지보다 중요한 것은 다음 업무 규칙이 DB에서 보장되는 것이다.

> **한 직원에게 같은 연도의 연차 집계 데이터는 하나만 존재해야 한다.**
